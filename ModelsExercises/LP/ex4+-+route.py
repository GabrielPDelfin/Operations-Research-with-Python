...
#Solo se puede salir de A por un camino
m.C1 = pyo.Constraint(expr = sum([m.x['A',j] for j in m.setRoutes_from['A']]) == 1)
#Solo se puede entrar a B por un camino
m.C2 = pyo.Constraint(expr = sum([m.x[i,'B'] for i in m.setRoutes_to['B']]) == 1)

#No vale hacer saltos durante el camino, debe estar todo el recorrido conectado. No se puede ir por varias rutas a la vez.
...
for route in m.setRoutes:
    if pyo.value(m.x[route[0], route[1]]) >= 0.9:
        ...
        print(pyo.value(m.x[route[0], route[1]]))